...
#include "actionlib_tutorials/forwardAction.h"
...
typedef actionlib::SimpleActionServer<actionlib_tutorials::forwardAction> Server;

//收到action的goal后调用该回调函数，该回调函数包含两个参数：forwardGoal的常量指针goal、服务器的指针as
void execute(const actionlib_tutorials::forwardGoalConstPtr& goal, Server* as)
{
    ...
    actionlib_tutorials::forwardFeedback feedback;
    actionlib_tutorials::forwardResult result;
